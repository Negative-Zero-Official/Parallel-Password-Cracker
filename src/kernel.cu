...
// A global id variable in device memory to locate the global id of the returning thread
__device__ unsigned long long d_found_id = 0;
    ...
    unsigned long long total_permutations)
{
    ...
    for (unsigned long long id = start_id; id < total_permutations; id += stride)
    {
        if (d_found_flag)
            return;
    ...
    unsigned long long *found_id)
{
    ...
    for (int i = 0; i < search_len; i++)
        total_permutations *= charset_len;
    ...
    if (h_found_flag && found_id != NULL)
    {
        cudaMemcpyFromSymbol(found_id, d_found_id, sizeof(unsigned long long));
    }
